...
pub fn load_config(path: &Path) -> Result<CmdexConfig> {
    if !path.exists() {
    ...
    let agents = raw
        ...
        .map(|raw_agent| {
            Ok(AgentDefinition {
                ...
                workspace: normalize_path(&raw_agent.workspace)?,
            })
        })
...
pub fn save_config(path: &Path, config: &CmdexConfig) -> Result<()> {
    let raw = RawConfig {
        agents: config
            .agents
            .iter()
            .map(|agent| RawAgentDefinition {
                ...
                workspace: compact_home(&agent.workspace),
            })
            .collect(),
    };
...
mod tests {
    ...
    fn compacts_home_paths() {
        ...
        assert_eq!(compact_home(&nested), "~/projects/example");
    }
...
"#;
        ...
        let agent = AgentDefinition {
            ...
            workspace: normalize_path(&raw.agents[0].workspace).expect("normalized"),
        };
