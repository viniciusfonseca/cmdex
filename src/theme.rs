...
static THEME_BUNDLE: LazyLock<ThemeBundle> = LazyLock::new(|| ThemeBundle {
    app: hardcoded_app_theme(),
    syntax: hardcoded_syntax_theme(),
});
...
fn hardcoded_app_theme() -> AppTheme {
    AppTheme {
        ...
        scrollbar_track: rgb(0x3E, 0x3E, 0x3C),
    }
}
...
fn hardcoded_syntax_theme() -> Theme {
    Theme {
        ...
        settings: ThemeSettings {
            foreground: Some(syn_rgb(0xE8, 0xE4, 0xDC)),
            background: Some(syn_rgb(0x26, 0x26, 0x24)),
            caret: Some(syn_rgb(0xF4, 0x84, 0x5F)),
            line_highlight: Some(syn_rgb(0x2E, 0x2E, 0x2C)),
            find_highlight: Some(syn_rgba(0xF4, 0x84, 0x5F, 0x44)),
            gutter: Some(syn_rgb(0x26, 0x26, 0x24)),
            gutter_foreground: Some(syn_rgb(0x4A, 0x4A, 0x48)),
            selection: Some(syn_rgba(0x3A, 0x3A, 0x38, 0x88)),
            inactive_selection: Some(syn_rgba(0x3A, 0x3A, 0x38, 0x44)),
            guide: Some(syn_rgb(0x33, 0x33, 0x31)),
            active_guide: Some(syn_rgb(0x3A, 0x3A, 0x38)),
            shadow: Some(syn_rgba(0x00, 0x00, 0x00, 0x33)),
            ..ThemeSettings::default()
        },
        scopes: vec![
            theme_item(
                "comment, punctuation.definition.comment",
                syn_rgb(0x5A, 0x5A, 0x58),
                None,
                Some("italic"),
            ),
            theme_item(
                "string, string.quoted, string.template",
                syn_rgb(0x7E, 0xC6, 0x99),
                None,
                None,
            ),
            theme_item("string.regexp", syn_rgb(0x7C, 0xE8, 0xD4), None, None),
            theme_item(
                "constant.numeric, constant.language, constant.character",
                syn_rgb(0xF4, 0xA5, 0x8A),
                None,
                None,
            ),
            theme_item("constant.other", syn_rgb(0xE8, 0xC4, 0x7C), None, None),
            theme_item(
            ...
            theme_item(
                ...
                syn_rgb(0x8A, 0x8A, 0x88),
                None,
                None,
            ),
            theme_item(
                "punctuation.separator, punctuation.terminator",
                syn_rgb(0x6A, 0x6A, 0x68),
                None,
                None,
            ),
            theme_item(
            ...
            theme_item("markup.bold", syn_rgb(0xE8, 0xE4, 0xDC), None, Some("bold")),
            theme_item(
                "markup.italic",
                syn_rgb(0xC4, 0x9B, 0xE8),
                None,
            ...
            theme_item("markup.list", syn_rgb(0xF4, 0xA5, 0x8A), None, None),
            theme_item(
                "markup.quote",
                syn_rgb(0x6A, 0x6A, 0x68),
                None,
...
fn theme_item(
    ...
    ThemeItem {
        ...
        style: StyleModifier {
            ...
            background,
            font_style: font_style.map(parse_font_style),
        },
    }
}
...
mod tests {
    ...
    fn embeds_expected_app_theme_colors() {
        ...
        assert_eq!(theme.app_bg, rgb(0x26, 0x26, 0x24));
        assert_eq!(theme.accent, rgb(0xF4, 0x84, 0x5F));
        assert_eq!(theme.yellow, rgb(0xE8, 0xC4, 0x7C));
    }
    ...
    fn embeds_syntax_theme_rules() {
        ...
        assert_eq!(theme.settings.background, Some(syn_rgb(0x26, 0x26, 0x24)));
    }
}
