...
fn build_diff_entries(root: &Path) -> Result<DiffSections> {
    ...
    Ok(parse_status_sections(root, &stdout))
}
...
pub(super) fn parse_status_sections(root: &Path, output: &str) -> DiffSections {
    ...
    for line in output.lines() {
        let Some((index_status, worktree_status, path)) = parse_status_line(line) else {
            continue;
        };
...
fn render_diff_snapshot_preview(path: &Path, snapshot: &DiffSnapshot) -> Vec<Line<'static>> {
    ...
    let mut lines = render_diff_preview_rows(build_diff_preview_rows(
        path,
        &snapshot.source,
...
pub(super) fn render_modified_diff_preview(
    ...
    render_diff_preview_rows(build_diff_preview_rows(path, source, None, hunks))
}
